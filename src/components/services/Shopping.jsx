...
export default function Shopping() {
  ...
  const service = useSelector((state) =>
    state.data.categories.find((s) => s.id === 3)
  ); // Service Shopping (id = 3)
  ...
  useEffect(() => {
    if (!children) return;
    const filteredData = children.filter((child) => child.parentId === 3);
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="grid grid-cols-2 gap-4">
        {data.map((childService) => (
          <div
            key={childService.id}
            onClick={() => navigate(`/service/shopping/${childService.id}`)}
          ...
          >
            <div className="rounded overflow-hidden mx-auto mb-3 border border-gray-200 shadow-sm w-full h-32">
              <img
                src={getImage(childService.image)}
                alt={childService.name}
                className="w-full h-full object-cover"
              />
            </div>
            <h3 className="font-medium text-gray-800 text-center">{childService.name}</h3>
          </div>
        ))}
      </div>
    </div>
